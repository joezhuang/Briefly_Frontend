...
export function ArticleView({article,immutable=false,podcast=null,podcastBusy=false,podcastPro=false,podcastSignedIn=false,onPodcastAction,briefRepair=null,briefRepairBusy=false,onBriefRepair,translationAction,refreshKey,focusCommunity=false,shareHref,community,footer}:{article:CanonicalArticle;immutable?:boolean;podcast?:PodcastAnalysisStatus|null;podcastBusy?:boolean;podcastPro?:boolean;podcastSignedIn?:boolean;onPodcastAction?:()=>void;briefRepair?:BriefRepairStatus|null;briefRepairBusy?:boolean;onBriefRepair?:()=>void;translationAction?:ReactNode;refreshKey?:string|number|null;focusCommunity?:boolean;shareHref?:string;community?:ReactNode;footer?:ReactNode;}){
  ...
  const share=async()=>{const url=buildPublicStoryShareUrl(article,shareHref);if(!url){Alert.alert("Briefly",t.shareConfigMissing);return}const result=await Share.share(Platform.OS==="ios"?{message:article.headline,url}:{message:`${article.headline}\n${url}`});if(result.action!==Share.dismissedAction){trackProductEvent("story_share",{eventId:article.event_id,articleVersionId:article.article_version_id,properties:{source:"story"}})}};
