...
export function ArticleView({article,immutable=false,podcast=null,podcastBusy=false,podcastPro=false,podcastSignedIn=false,onPodcastAction,briefRepair=null,briefRepairBusy=false,onBriefRepair,translationAction,refreshKey,focusCommunity=false,community,footer}:{article:CanonicalArticle;immutable?:boolean;podcast?:PodcastAnalysisStatus|null;podcastBusy?:boolean;podcastPro?:boolean;podcastSignedIn?:boolean;onPodcastAction?:()=>void;briefRepair?:BriefRepairStatus|null;briefRepairBusy?:boolean;onBriefRepair?:()=>void;translationAction?:ReactNode;refreshKey?:string|number|null;focusCommunity?:boolean;community?:ReactNode;footer?:ReactNode;}){
  ...
  const [openExplore,setOpenExplore]=useState<"sources"|"coverage"|null>(null);const [showFloatingBack,setShowFloatingBack]=useState(false);const scrollRef=useRef<ScrollView|null>(null);const communityYRef=useRef<number|null>(null);const communityFocusDoneRef=useRef(false);const [briefExpansion,setBriefExpansion]=useState<{articleKey:string;sections:Partial<Record<BriefSectionId,boolean>>}>({articleKey:"",sections:{}});const podcastProcessing=podcastBusy||podcast?.status==="processing";const podcastReady=podcast?.status==="ready"&&!!podcast.audio_url;
  ...
  useEffect(()=>{communityFocusDoneRef.current=false;communityYRef.current=null},[article.event_id,focusCommunity]);
  ...
  const share=async()=>{const webBase=process.env.EXPO_PUBLIC_BRIEFLY_WEB_URL?.replace(/\/$/,"");if(!webBase){Alert.alert("Briefly",t.shareConfigMissing);return}const url=`${webBase}/share/${article.article_version_id}`;const result=await Share.share(Platform.OS==="ios"?{message:article.headline,url}:{message:`${article.headline}\n${url}`});if(result.action!==Share.dismissedAction){trackProductEvent("story_share",{eventId:article.event_id,articleVersionId:article.article_version_id,properties:{source:"story"}})}};
