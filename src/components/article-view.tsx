...
import { ActivityIndicator, Alert, Linking, Platform, Pressable, ScrollView, StyleSheet, Text, useWindowDimensions, View } from "react-native";
...
const exploreCopy={en:{title:"Explore this event",sources:"Sources used",coverage:"Coverage"},es:{title:"Explora este evento",sources:"Fuentes utilizadas",coverage:"Cobertura"},ja:{title:"この出来事を詳しく見る",sources:"使用した情報源",coverage:"関連記事"},"zh-CN":{title:"深入了解这一事件",sources:"使用的来源",coverage:"相关报道"},"zh-TW":{title:"深入了解這一事件",sources:"使用的來源",coverage:"相關報導"}} as const;
...
export function ArticleView({article,immutable=false,podcast=null,podcastBusy=false,podcastPro=false,podcastSignedIn=false,onPodcastAction,briefRepair=null,briefRepairBusy=false,onBriefRepair,translationAction,refreshKey,focusCommunity=false,autoStartVideo=false,shareHref,community,footer}:{article:CanonicalArticle;immutable?:boolean;podcast?:PodcastAnalysisStatus|null;podcastBusy?:boolean;podcastPro?:boolean;podcastSignedIn?:boolean;onPodcastAction?:()=>void;briefRepair?:BriefRepairStatus|null;briefRepairBusy?:boolean;onBriefRepair?:()=>void;translationAction?:ReactNode;refreshKey?:string|number|null;focusCommunity?:boolean;autoStartVideo?:boolean;shareHref?:string;community?:ReactNode;footer?:ReactNode;}){
  ...
  const [openExplore,setOpenExplore]=useState<"sources"|"coverage"|null>(null);const [showFloatingBack,setShowFloatingBack]=useState(false);const scrollRef=useRef<ScrollView|null>(null);const communityYRef=useRef<number|null>(null);const communityFocusActiveRef=useRef(false);const [briefExpansion,setBriefExpansion]=useState<{articleKey:string;sections:Partial<Record<BriefSectionId,boolean>>}>({articleKey:"",sections:{}});const podcastProcessing=podcastBusy||podcast?.status==="processing";const podcastReady=podcast?.status==="ready"&&!!podcast.audio_url;
  ...
  useEffect(()=>{communityYRef.current=null;communityFocusActiveRef.current=focusCommunity},[article.event_id,focusCommunity]);
  ...
  const share=async()=>{const url=buildPublicStoryShareUrl(article,shareHref);if(!url){Alert.alert("Briefly",t.shareConfigMissing);return}const result=await shareBrieflyStory({headline:article.headline,url});if(result==="copied"){if(Platform.OS==="web"&&typeof window!=="undefined")window.alert(t.shareLinkCopied);else Alert.alert("Briefly",t.shareLinkCopied)}if(result!=="dismissed"){trackProductEvent("story_share",{eventId:article.event_id,articleVersionId:article.article_version_id,properties:{source:"story"}})}};
  const openCoverage=async(item:ArticleCoverage)=>{trackProductEvent("source_open",{eventId:article.event_id,articleVersionId:article.article_version_id,properties:{surface:"story_coverage",source:item.source,language:item.language??null}});if(Platform.OS==="web"&&typeof window!=="undefined"){window.open(item.url,"_blank","noopener,noreferrer");return}await Linking.openURL(item.url)};
  ...
  return <View style={[styles.articleRoot,{backgroundColor:colors.surface}]}><ScrollView ref={scrollRef} style={styles.screen} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==="ios"?"interactive":"on-drag"} onScroll={(event)=>{if(!immutable)setShowFloatingBack(event.nativeEvent.contentOffset.y>420)}} onScrollBeginDrag={()=>{communityFocusActiveRef.current=false}} onContentSizeChange={()=>{if(focusCommunity&&communityFocusActiveRef.current&&communityYRef.current!==null){const y=communityYRef.current;requestAnimationFrame(()=>scrollRef.current?.scrollTo({y:Math.max(0,y-16),animated:false}))}}} scrollEventThrottle={120}><View style={[styles.page,width<480&&styles.pageCompact]}>
    {hasHeroMedia&&<View style={[styles.heroFrame,{backgroundColor:colors.imageFallback}]}>{videoUrl?<StoryVideo url={videoUrl} posterUrl={heroImageUrl} accessibilityLabel={mediaText.play} autoStart={autoStartVideo}/>:heroImageUrl?<Image source={{uri:heroImageUrl}} style={styles.heroImage} contentFit="cover" transition={180}/>:null}</View>}
